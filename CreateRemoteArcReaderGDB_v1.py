...
# Created by Kim Sundeen 1/15/2016

"""
VERBOSE DESCRIPTION:
Here's the start of a python program to automate the copy & export of an SDE enterprise
...
#3.	Work with IT to add a pop-up message on field worker’s laptops that informs them if their
    database & map is more than 1 week old (& hopefully encourages them to update the map ASAP).
...
import arcpy, os, shutil, logging, time, datetime, gc
...
arcpy.env.workspace = "G:/PROJECTS/ArcReaderTest/PortableDuluth.gdb"
...
def createEmpytGDB(directoryPath='G:/PROJECTS/ArcReaderTest/',
    ...
    #try:
    # list directory & find all files ending in *.gdb & *.pmf
        #os.chdir("S:/GIS_Public/GIS_Data/MapDocuments/Published_Maps/TestArea/")
        #os.chdir(cleanupFolder)
...
### Run function
##createEmpytGDB(directoryPath = 'G:/PROJECTS/ArcReaderTest/', originalGDB = 'PortableGIS.gdb', backupNameGDB = 'PortableGIS_backup.gdb')
...
def copyFeatureDatasets(fdList, toGDBpath):
    ...
    toGDBpath = the full file path to gdb, such as 'S:/GIS_Public/GIS_Data/MapDocuments/Published_Maps/TestArea/PortableDuluth.gdb'
...
### Run Function
##outGDB = r'G:\PROJECTS\ArcReaderTest\PortableGIS.gdb'
##copyFeatureDatasets(fdList = fdList, toGDBpath = outGDB)
...
### Run function to update Assessor's table
##toGDBpath = r'G:\PROJECTS\ArcReaderTest\PortableGIS.gdb'
##updateAssessorTable(toGDBpath)
...
def clipAndCopyRiceLakeFC(toGDB_RiceLake_path = 'S:/GIS_Public/GIS_Data/MapDocuments/Published_Maps/PortableDuluth.gdb/Rice_Lake_Twnshp'):
    """
    PURPOSE:
    ...
    **Default Portable GDB filepath is specified in function declaration-->replace with new location, if needed
    toGDB_RiceLake_path = 'S:/GIS_Public/GIS_Data/MapDocuments/Published_Maps/PortableDuluth.gdb/Rice_Lake_Twnshp'
    """
    ...
    try:
        ...
        if arcpy.Exists(countyServDbs) == True:
            ...
            boundaryRiceLakeFC = 'S:/GIS_Public/GIS_Data/DefaultGDB/RiceLakeTownship_Clip.gdb/RiceLakeTownshipClipBoundary'
            ...
            for inLyr, outLyr in clipDict.iteritems():
                ...
                print 'Successfully clipped feature class: ({0}) by Rice Lake southern boundary ({1}) into PortableGIS.gdb: ({2})'.format(inLyr, boundaryRiceLakeFC, outLyr)
...
### Run function
##clipAndCopyRiceLakeFC(toGDB_RiceLake_path = 'G:/PROJECTS/ArcReaderTest/PortableGIS.gdb/Rice_Lake_Twnshp')
...
createEmpytGDB(directoryPath = 'G:/PROJECTS/ArcReaderTest/',
               originalGDB = 'PortableGIS.gdb',
               backupNameGDB = 'PortableGIS_backup.gdb')
...
# 3. Run function to take dictionary of keys (feature datasets) mapped to values of feature classes &
...
#------Script End------------------------------------------------------------------------------
print "----------------------------------------------"
print "\nScript completed in {0:0.2f} minutes (or {1:.2f} \
...
# consider other ArcReader projects that use this PortableGDB
## Surveyors Map-this one includes additional tables for surveying


# Program to create Tkinter buttons to run other programs
# Use Tkinter module to create user interface
##### Buttons included:
###### Update
###### Backup?
###### Submit ArcReader issue or question
# Export regular File GDB
# Tkinter button for running functions to copy & export new regular file GDB
##top = Tkinter.Tk()
##def exportRegularGDB():
##    tkMessageBox.showinfo("Admin Menu", "Successfully exported regular GDB")
##def exportSurveyGDB():
##    tkMessageBox.showinfo("Admin Menu", "Successfully exported Survey GDB")
##
##B = Tkinter.Button(top, text = "Export GDB?", command = exportRegularGDB)
##B.pack()
##
##C = Tkinter.Button(top, text = "Export Survey GDB?", command = exportSurveyGDB)
##C.pack()
##top.mainloop()


# BATCH FILE STUFF...
# 5. Richard suggests deleting all files in "DuluthGIS" folder (1 time)
# 5.1 then continue with copying over all other files
# 6. Delete any older ArcReader files on laptops
# 7. Copy over new ArcReader files
# 8. Work with IT to figure out how to push updates of file gdb/ArcReader to remote laptops
